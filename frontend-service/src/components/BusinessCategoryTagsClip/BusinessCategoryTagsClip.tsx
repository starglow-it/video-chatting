...
const Component = ({
    ...
    const hideData = useMemo(() => {
        ...
        return initialArray?.reduce((acc, element, index) => {
            ...
            const findLineIndex = acc.findIndex(line => line.overallWidth + rect.width < maxWidth);
            ...
            hideData.slice(0, lines).map((elementsData, index) => (
                <CustomGrid container gap={1.25}>
                    ...
                    {index === lines - 1 && Boolean(hideData[lines]?.elements?.length) && (
                        ...
                        >
                            <CustomTypography variant="body2" className={styles.numberOfHiddenTags}>
...
export const BusinessCategoryTagsClip = memo(Component);
