...
const Component: React.FunctionComponent<ChooseSignBoardProps> = ({ optionWidth, optionHeight }) => {
    const { control, register } = useFormContext();

    const renderSignOptions = useMemo(
        () =>
            SIGN_BOARDS.map(signOption => (
                <SignBoardOption key={signOption.id} data={signOption} width={optionWidth} height={optionHeight} />
            )),
        [],
    );
