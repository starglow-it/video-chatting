    ...
    const onDetectionResult = useCallback((results) => {
        ...
        if (canvasCtx) {
            ...
            if (results?.detections?.length > 0) {
                ...
                const heightPoint = results.image.height * results.detections[0].boundingBox.yCenter;

                const xFallback = results.detections[0].boundingBox.xCenter > 0.5 ? results.image.width - resultWidth : 0;
                const yFallback = results.detections[0].boundingBox.yCenter > 0.5 ? results.image.height - resultWidth : 0;

                const xResult = results.image.width - widthPoint < resultWidth / 2 || widthPoint < resultWidth / 2 ? xFallback : widthPoint - (resultWidth / 2);
                const yResult = results.image.height - heightPoint < resultWidth / 2 || heightPoint < resultWidth / 2 ? yFallback : heightPoint - (resultWidth / 2);
