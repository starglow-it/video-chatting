...
camera.lookAt(
    new THREE.Vector3(config.camera.lookAt.x, config.camera.lookAt.y, config.camera.lookAt.z),
);
scene.add(camera);
    ...
    if (config.camera.mouseTilt) {
        gsap.to(camera.position, {
            ...
            x:
                ...
                pointerConvert(e, canvas).x * config.camera.tiltSensibility,
            y:
                config.camera.position.y +
                pointerConvert(e, canvas).y * config.camera.tiltSensibility,
        });
    }
};
...
export const mobileCameraTilt = e => {
    gsap.to(camera.position, {
        duration: config.camera.tiltTime,
        x: config.camera.position.x + e.x * config.camera.tiltSensibility,
        y: config.camera.position.y + e.y * config.camera.tiltSensibility,
    });
};
