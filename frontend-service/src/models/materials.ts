    ...
    if (child.material.userData.shader) {
        ...
        if (data === 'background') {
            import('./shaders/background/Background').then(({ backgroundShader }) => {
                ...
                child.material = mat;
                load_image('/templates/models/models/static/textures/back512.jpg').then(
                    response => {
                ...
                load_image(
                    '/templates/models/models/static/textures/background_layers_png.png',
                ).then(response => {
                    response.flipY = false;
                    child.material.uniforms.uBackgroundLayers.value = response;
                });
                load_image('/templates/models/models/static/textures/fnoise.png').then(response => {
                    response.wrapS = THREE.RepeatWrapping;
                    response.wrapT = THREE.RepeatWrapping;
                    child.material.uniforms.uFnoise.value = response;
                });
                load_image('/templates/models/models/static/textures/cellnoise.png').then(
                    response => {
                        response.wrapS = THREE.RepeatWrapping;
                        response.wrapT = THREE.RepeatWrapping;
        ...
        if (data === 'fire') {
            if (!mobileAndTabletCheck()) {
                ...
                createVideoTexture(
                    '/templates/models/videoTextures/',
                    'fire',
                    'ogg',
                    child.material,
                    1,
        ...
        if (data === 'main') {
            import('./shaders/main/Main').then(({ mainShader }) => {
                ...
                load_image('/templates/models/models/static/textures/texture.png').then(
                    response => {
                        response.flipY = false;
                        mat.uniforms.uLight.value = response;
                    },
                );
                load_image('/templates/models/models/static/textures/shadow.jpg').then(response => {
                    response.flipY = false;
                    mat.uniforms.uDark.value = response;
                });
                load_image('/templates/models/models/static/textures/shadow_mask.jpg').then(
                    response => {
                        response.flipY = false;
                        mat.uniforms.uMask.value = response;
                    },
                );
                load_image('/templates/models/models/static/textures/passes/uv.jpg').then(
                    response => {
                        response.flipY = false;
                        mat.uniforms.uNormal.value = response;
                    },
                );
                load_image('/templates/models/models/static/textures/passes/z_denoise.jpg').then(
                    response => {
                        response.flipY = false;
        ...
        if (data === 'smoke') {
            if (!mobileAndTabletCheck()) {
                import('./shaders/smoke/Smoke').then(({ SmokeShader }) => {
                    ...
                    child.material = mat;
                    child.material.index0AttributeName = 'position';
                    if (child.userData.smokeIntensity)
                        child.material.uniforms.uIntensity.value = child.userData.smokeIntensity;
                    if (child.userData.smokeX)
                        child.material.uniforms.uX.value = child.userData.smokeX;
                    if (child.userData.smokeY)
                        child.material.uniforms.uY.value = child.userData.smokeY;
                    if (child.userData.offset)
                        child.material.uniforms.uOffset.value = child.userData.offset;
                    uTimeArrays.push(child.material);
                });
            } else {
                //create video html and then catch it
