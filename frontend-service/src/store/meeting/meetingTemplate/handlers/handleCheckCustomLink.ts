...
export const handleCheckCustomLink = async ({
   templateId,
}: { templateId: UserTemplate['id'] | UserTemplate['customLink'] }): Promise<boolean> => {
    const response = await sendRequest<UserTemplate, ErrorState>(getUserTemplatesUrl({ templateId }));
