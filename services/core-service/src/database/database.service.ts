...
export class DatabaseService implements OnModuleInit, OnApplicationBootstrap {
  ...
  private async getRecords(entities: string[]): Promise<Collection[]> {
    try {
      return (await Promise.all(
        entities.map(async (modelName) => {
          const schemaObject = this.models[modelName]?.schema.obj || {};

          const fieldList = Object.keys(schemaObject);

          const filterFields = fieldList.filter(
            (field) => typeof schemaObject[field]['default'] !== 'undefined',
          );
        ...
        collections.map(async (collection) => {
          collection.records.map(async (record) => {
            const updateData = this.handleMappingValuesByKeys(
              ...
              record.missKeys.map(
                (missKey) =>
                  this.models[collection.name].schema.obj[missKey]['default'],
              ),
            ).object;

            await this.models[collection.name].findByIdAndUpdate(record._id, {
              $set: {
                ...updateData,
              },
            });
  ...
  private handleMappingValuesByKeys(keysArr: string[], value: any): MappingKey {
    try {
      const mappingValue = (cur, index) => ({
        [cur]: isArray(value) ? value[index] : value,
      });
      return {
        list: keysArr
          .filter((item) => this.isAcceptFilter(item))
          .reduce((acc, cur, index) => [...acc, mappingValue(cur, index)], []),
        object: keysArr
          .filter((item) => this.isAcceptFilter(item))
          .reduce(
            (acc, cur, index) => ({
  ...
  private async syncDataLocal() {
    try {
      const entities = ['User', 'CommonTemplate', 'UserTemplate'];
