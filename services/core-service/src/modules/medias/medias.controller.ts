...
export class MediaController {
  ...
  private async getMediaQuery({
    ...
    if (userTemplateId) {
      const userTemplate = await this.getUserTemplate({
        userTemplateId,
        session,
      });
      userTemplateQuery = {
  ...
  private async getMediaCategories({
    query,
    skip,
    limit,
    session,
  ...
  async getMedias(
    @Payload() payload: GetMediasPayload,
  ): Promise<EntityList<IMedia>> {
    return withTransaction(this.connection, async (session) => {
      try {
  ...
  async createMediaCategory(
    @Payload() { key, type, value, emojiUrl }: CreateMediaCategoryPayload,
  ) {
    try {
  ...
  async createUserTemplateMedia(
    @Payload() { categoryId, userTemplateId }: CreateMediaPayload,
  ) {
    return withTransaction(this.connection, async (session) => {
      try {
        let userTemplate = null;
        if (userTemplateId) {
          userTemplate = await this.getUserTemplate({
            userTemplateId,
            session,
          });
        }
  ...
  async updateMedia(@Payload() { id, data }: UpdateMediaPayload) {
    return withTransaction(this.connection, async (session) => {
      const media = await this.mediaService.updateMedia({
        query: {
          _id: id,
  ...
  async updateMediaCategory(
    @Payload() { id, data }: UpdateMediaCategoryPayload,
  ) {
    return withTransaction(this.connection, async (session) => {
      const category = await this.mediaService.updateMediaCategory({
        query: {
  ...
  async deleteMediaCategories(
    @Payload() payload: DeleteMediaCategoriesPayload,
  ): Promise<void> {
    return withTransaction(this.connection, async (session) => {
      try {
        const { ids } = payload;
  ...
  async deleteMedias(
    @Payload() { ids, categoryId, userTemplateId }: DeleteMediasPayload,
  ): Promise<void> {
    return withTransaction(this.connection, async (session) => {
      try {
        const mediaCategory = await this.mediaService.findMediaCategory({
