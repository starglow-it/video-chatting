...
// shared
import {ACCEPT_MIMES_AUDIO, MAX_SIZE_AUDIO_MB} from "shared-const";
...
import {NotificationType, Notification } from "../../../store/types";
...
export const TemplateSound = memo(({ src, isUploadDisabled, fileName, onRemove, onNextStep, onPreviousStep, onFileUploaded }: TemplateSoundProps) => {
    const [localFileName, setLocalFileName] = useState('');

    const handleTemplateSoundUploaded = useCallback((files: File[], fileRejections: FileRejection[]) => {
        let errorMessage: Notification = {
            type: NotificationType.UploadFileFail,
            withErrorIcon: true,
            message: '',
        };

        const isMimeError = fileRejections[0]?.errors?.[0]?.code === 'file-invalid-type';
        const isSizeError = fileRejections[0]?.errors?.[0]?.code === 'file-too-large';

        if (files.length > 1) {
            errorMessage.message = 'uploadBackgroundSound.manyFiles';
        } else if (isMimeError) {
            errorMessage.message = 'uploadBackgroundSound.invalidFormat';
        } else if (isSizeError) {
        ...
        >
            ...
            {src ? (
                ...
                <>
                    {isDragActive
                        ? (
                            <UploadDragFileOverlay
                                title="uploadBackgroundAudio.title"
                            />
                        )
                        : (
                            <CustomPaper
                            ...
                            >
                                <CustomGrid container direction="column" justifyContent="center" alignItems="center">
                                    <CustomTypography textAlign="center" variant="h2bold" color="colors.white.primary">
                                        <Translation nameSpace="rooms" translation="uploadSound.title"/>
                                    </CustomTypography>
                                    <CustomTypography textAlign="center" color="colors.grayscale.normal">
                                        <Translation nameSpace="rooms" translation="uploadSound.text"/>
                                    </CustomTypography>
                                    <CustomButton
                                        ...
                                        onClick={onClick}
                                        label={(
                                            <Translation nameSpace="common" translation="buttons.upload"/>
                                        )}
                                    />
                                </CustomGrid>
                            </CustomPaper>
