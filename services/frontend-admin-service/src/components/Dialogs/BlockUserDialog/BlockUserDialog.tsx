...
const Component = () => {
    ...
    const handleBlockUser = useCallback(() => {
        closeAdminDialogEvent(AdminDialogsEnum.blockUserDialog);
        blockUserFx({ userId: blockUserId, isBlocked: !userData?.isBlocked });
        setBlockUserId(null);
        ...
        >
            <CustomGrid container alignItems="center" justifyContent="center">
                <CustomTypography variant="h4">
                    <Translation nameSpace="users" translation={`dialogs.${userData?.isBlocked ? "unblockUser" : "blockUser" }.title`} />
                </CustomTypography>
                &nbsp;
            ...
            <CustomGrid className={styles.buttons} container alignItems="center" wrap="nowrap" gap={2}>
                <CustomButton
                    variant={userData?.isBlocked ? "custom-black" : "custom-danger"}
                    onClick={handleBlockUser}
                    label={
                        <Translation
                            nameSpace="common"
                            translation={userData?.isBlocked ? "buttons.unblock" : "buttons.block" }
                        />
                    }
                />
                <CustomButton
