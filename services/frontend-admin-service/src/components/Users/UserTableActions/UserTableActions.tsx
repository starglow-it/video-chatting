...
// shared
import {ActionButton, CustomGrid, CustomTooltip, LockIcon, TrashIcon} from "shared-frontend";
import {Translation} from "@components/Translation/Translation";

// stores
import {$usersStore, setBlockUserId, setDeleteUserId, openAdminDialogEvent} from "../../../store";
...
const Component = ({ actionId }: { actionId: string }) => {
    ...
    return (
        <CustomGrid container alignItems="center" justifyContent="center" gap={1}>
            <CustomTooltip
                title={
                    <Translation
                        nameSpace="common"
                        translation={`tooltips.users.${userData?.isBlocked ? "unblock" : "block"}`}
                    />
                }
                placement="bottom"
            >
                <ActionButton
                    className={clsx(styles.actionButton, {[styles.blocked]: userData?.isBlocked })}
                    onAction={handleBlockUser}
                    variant="decline"
                    Icon={<LockIcon width="18px" height="18px" />}
                />
            </CustomTooltip>
            <CustomTooltip
                title={
