import { statisticsScope, usersScope } from 'shared-const';
...
export const userProfileUrl = ({ userId }: GetUserProfileParams) => ({
    url: `${serverUrl}/${usersScope}/${userId}`,
    method: HttpMethods.Get,
});

export const userProfileStatisticUrl = ({ userId }: GetUserProfileStatisticsParams) => ({
...
export const searchUsersUrl = ({ skip = 0, limit = 0, search }: QueryParams) => ({
    url: `${serverUrl}/${usersScope}/search?skip=${skip}&limit=${limit}&search=${search}`,
    method: HttpMethods.Get,
});

export const blockUserUrl = ({ userId, key, value }: { userId: string; key: string; value: boolean }) => ({
    url: `${serverUrl}/${usersScope}/rights/${userId}?key=${key}&value=${value}`,
    method: HttpMethods.Post,
});

export const deleteUserUrl = ({ userId }: { userId: string; }) => ({
    url: `${serverUrl}/${usersScope}/${userId}`,
