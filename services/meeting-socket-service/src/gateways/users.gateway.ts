...
export class UsersGateway extends BaseGateway {
  ...
  private dateFormat = (value, country) => {
    ...
    const countryInstance: { id: string, name: string, timezones: string[] } | undefined =
      Object.values(allCountries).find((countryObj: { id: string, name: string, timezones: string[] }) => countryObj.name === country);
